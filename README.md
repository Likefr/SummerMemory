...
基于真实使用场景的测试数据（54 个记忆文件，270 个 chunks）：

| 指标 | 数值 | 说明 |
|:---|:---|:---|
| 📊 总文件数 | 54 个 | 记忆文件数量 |
| 📦 总 chunks | 270 个 | 分块后的记忆片段 |
| ⚡ 搜索速度 | **12-425ms** | 含向量计算 + 数据库查询 |
| 🧬 向量维度 | 512 维 | bge-small-zh-v1.5 |
| 💾 数据库大小 | ~50MB | 270 条记录含向量索引 |
| 🔄 索引速度 | ~2s/文件 | 含 embedding 生成 |
| 💰 **总成本** | **¥0** | **纯本地，零费用** |

> 💡 搜索延迟 12-425ms，比网络请求到商业 API 的延迟还低！
...
| ⚡ **搜索速度** | ✅ 12-425ms | 100-500ms | 50-200ms + 网络延迟 |
| 📊 **图谱可视化** | ✅ D3.js 力导向图 | ❌ 无 | ⚠️ 需额外付费 |
| 🔧 **可定制性** | ✅ 完全可控 | ❌ 受限 | ⚠️ API 受限 |
| 🐳 **部署难度** | ✅ Docker 一键 | ✅ 内置 | ⚠️ 接入配置 |
| 🤖 **AI 集成** | ✅ OpenClaw Skill | ✅ 原生 | ⚠️ 需适配 |

### SummerMemory 的独特优势

1. **🇨🇳 中文深度优化**：jieba 分词 + 中文向量模型，不是简单套用英文方案
2. **🔗 真正的混合搜索**：向量语义 + BM25 精确，两种引擎互相补位
3. **🛡️ 智能降级**：Ollama 挂了？自动回退到 BM25，搜索不中断
4. **🕸️ 知识图谱**：记忆不是孤立的，可视化展示关联关系
...
| **中文分词** | jieba | 中文全文检索分词 |
| **混合搜索** | pgvector 余弦相似度 + PostgreSQL TSVECTOR BM25 | 双引擎融合搜索 |
